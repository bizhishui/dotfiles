...
if [ "$1" = "S" ]; then
    ...
    rsync -ravh -e ssh  --exclude='in' --exclude='out' --exclude='wall'  --include='*.vtk' --include='*.txt' --exclude='*.ser' --exclude='RUN*' jlv@login.ccamu.u-3mrs.fr:$2 .
#    rsync -ravh --progress -e ssh --include='*.vtk' --include='*.txt' --exclude='*.ser' jlv@login.ccamu.u-3mrs.fr:$2 .
elif [ "$1" = "SS" ]; then
    echo "Only copy txt files"
    rsync -ravh -e ssh  --exclude='in' --exclude='out' --exclude='wall' --exclude='*.vtk' --include='*.txt' --exclude='*.ser' --exclude='*.jar' --exclude='RUN*' jlv@login.ccamu.u-3mrs.fr:$2 .
#    rsync -ravh --progress -e ssh --exclude='*.vtk' --include='*.txt' --exclude='*.ser' --exclude='*.jar' jlv@login.ccamu.u-3mrs.fr:$2 .
elif [ "$1" = "F" ]; then
    echo "Copy *.ser files"
    rsync -ravh -e ssh --include='*.vtk' --include='*.txt' --include='*.ser'  jlv@login.ccamu.u-3mrs.fr:$2 .
else
